...
# ####################################################################
# (base) ➜  CodeR git:(main) ✗ ./metagenomics_Integrated_version3.R -h
# usage: metagenomics_Integrated_version3.R [--] [--help] [--opts OPTS]
...
# [--fasta_dir FASTA_DIR] [--export EXPORT] [--align ALIGN]
# [--venn_viz VENN_VIZ]
# 
...
roary_phylogeny <- read.tree(TreeLine(myXStringSet =  readDNAStringSet(file = "test.fasta"), method = "ML", 
                           cutoff = 0.05, showPlot = TRUE, reconstruct = TRUE))                         
...
plot_pangenome_params(panstr_gau) + plot_pangenome_cumulative(panstr_gau) + plot_layout(nrows = 1)
roary_most_variable_genes <- colnames(roary_pan)[apply(roary_pan, 2, sd) > 0]
